...
# from src.model.sklearn_cdnmf import _update_cdnmf_fast
...
def _beta_divergence(X, U, W, H, beta, square_root=False):
    """Compute the beta-divergence of X and dot(W, H) divided by U.

    Parameters
    ----------
    X : float or array-like of shape (n_samples, n_features)

    U : float or array-like of shape (n_samples, n_features)
    ...
    if beta == 2:
        ...
        if sp.issparse(X):
            ...
            # res = squared_norm((X - np.dot(W, H)) / U)      # NMF-PY uncertainty loss
            # return res
...
def _q_multiplicative_update_h(X, W, H, l1_reg_H, l2_reg_H):
    """

    :return:
    """
    numerator = np.matmul(W.T, X)
...
def _q_multiplicative_update_w(X, W, H, l1_reg_W, l2_reg_W):
    """
    ...
    :return:
    """
    numerator = np.matmul(X, H.T)
    denominator = np.linalg.multi_dot([W, H, H.T])
...
def _fit_multiplicative_update(
    ...
    U : array-like of shape (n_samples, n_features)
        Constant uncertainty of input matrix
    ...
    if q_loss:
        error_at_init = _q_divergence(X, U, W, H)
    else:
        error_at_init = _beta_divergence(X, U, W, H, beta_loss, square_root=True)
    previous_error = error_at_init

    prior_errors = []
    n_prior_errors = n_prior_errors
    error_tol = error_tol

    H_sum, HHt, XHt = None, None, None

    t_iter = trange(max_iter+1, desc=f"Q = NA", leave=True, disable=False)
    # for n_iter in range(1, max_iter + 1):
    for n_iter in t_iter:
        if q_loss:
            H = _q_multiplicative_update_h(
                X,
                W,
                H,
                l1_reg_H=l1_reg_H,
        ...
        if tol > 0 and n_iter % 10 == 0:
            if q_loss:
                error = _q_divergence(X, U, W, H)
            else:
                error = _beta_divergence(X, U, W, H, beta_loss, square_root=True)
            ...
            if len(prior_errors) == n_prior_errors:
                prior_errors.pop(0)
            prior_errors.append(error)
            if q_loss:
                if (max(prior_errors) - min(prior_errors)) < error_tol and len(prior_errors) == n_prior_errors:
                    break
            else:
            ...
            t_iter.set_description(f"Step: {n_iter + 1} Q = {round(error, 2)}, initial Q:{round(error_at_init, 2)} ")
            t_iter.refresh()
...
def non_negative_factorization(
    ...
    *,
    ...
    U = check_array(U, accept_sparse=("csr", "csc"), dtype=[np.float64, np.float32])
    ...
    with config_context(assume_finite=True):
        W, H, n_iter = est._fit_transform(X, U=U, W=W, H=H, update_H=update_H)
...
class NMF(_ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def __init__(
        ...
        *,
        ...
        self.n_prior_errors = n_prior_errors
    ...
    def fit_transform(self, X, U=None, y=None, W=None, H=None):
        ...
        U : {array-like, sparse matrix} of shape (n_samples, n_features)
            Uncertainty of training data vector, where `n_samples` is the number of samples
            and `n_features` is the number of features.
        ...
        U = self._validate_data(
            U, accept_sparse=("csr", "csc"), dtype=[np.float64, np.float32]
        )

        with config_context(assume_finite=True):
            W, H, n_iter = self._fit_transform(X, U, W=W, H=H)

        if self.q_loss:
            self.reconstruction_err_ = _q_divergence(X, U, W, H)
        else:
            self.reconstruction_err_ = _beta_divergence(
                X, U, W, H, self._beta_loss, square_root=True
            )
        ...
        self.H = H
        self.WH = np.dot(W, H)
        self.Qrobust = self.reconstruction_err_

        return W

    def _fit_transform(self, X, U=None, y=None, W=None, H=None, update_H=True):
        """Learn a NMF model for the data X and returns the transformed data.
        ...
        U : {array-like, spare matrix} of shape (n_samples, n_features)
            Uncertainty of data matrix
        ...
        if self.solver == "cd":
            ...
            W, H, n_iter, *_ = _fit_multiplicative_update(
                ...
                self.verbose,
                q_loss=self.q_loss,
                n_prior_errors=self.n_prior_errors,
...
class MiniBatchNMF(NMF):
    ...
    def _minibatch_step(self, X, U, W, H, update_H):
        ...
        batch_cost = (
            _beta_divergence(X, U, W, H, self._beta_loss)
            + l1_reg_W * W.sum()
            + l1_reg_H * H.sum()
            + l2_reg_W * (W**2).sum()
            + l2_reg_H * (H**2).sum()
    ...
    def fit_transform(self, X, U, y=None, W=None, H=None):
        """Learn a NMF model for the data X and returns the transformed data.
    ...
    def _fit_transform(self, X, U, W=None, H=None, update_H=True):
        """Learn a NMF model for the data X and returns the transformed data.
        ...
        U : {ndarray, sparse matrix} of shape (n_samples, n_features)
            Uncertainty data matrix.
        ...
        for i, batch in zip(range(n_steps), batches):
            ...
            batch_cost = self._minibatch_step(X[batch], U[batch], W[batch], H, update_H)
