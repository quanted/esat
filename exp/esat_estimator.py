...
class FactorCountOptimizer:
    ...
    def run_scenario(self, V=None, U=None, true_factors: int = None, k_min: int = 3, k_max: int = 12, scenario_id=None):
        ...
        for factors, fc in k_catalog.items():
            ...
            range_index.append(factors)
            range_catalog.append({
        ...
        metrics_df = pd.DataFrame(range_catalog, index=range_index)
    ...
    def train(self, n_scenarios=1000, k_min=3, k_max=12):
        ...
        with tqdm(range(n_scenarios), desc="Training scenarios", leave=True) as pbar:
            for i in range(n_scenarios):
                batch_metrics_df, i_factors = self.run_scenario(k_min=k_min, k_max=k_max)
                ...
                accuracy = [x == y for x, y in zip(predictions, [i_factors])]
                difference = np.mean(predictions - i_factors)
                error = np.sum(np.abs(predictions - i_factors)) / len(predictions)
                ...
                batch_metrics_df['is_optimal'] = batch_metrics_df['true_factors'] == i_factors
                ...
                grouped_data[i] = batch_metrics_df.assign(scenario_id=i)
                pbar.update(1)
    ...
    def predict_optimal_factor_count(self, metrics_df, confidence_method='ensemble'):
        ...
        optimal_row = metrics_df.iloc[optimal_idx]
        optimal_factor_count = optimal_row['true_factors']
        max_anomaly_score = optimal_row['anomaly_score']
        ...
        return {
            ...
            'all_scores': metrics_df[['true_factors', 'anomaly_score']].to_dict('records')
        }
...
def demonstrate_generalized_model(test_k:int = 5):
    ...
    plt.bar(scores_df['true_factors'], scores_df['anomaly_score'])
    ...
    plt.subplot(2, 3, 2)
    training_df = pd.concat(optimizer.grouped_data.values(), ignore_index=True)
    training_grouped = training_df.groupby(['true_factors', 'is_optimal'])['anomaly_score'].mean().unstack()
    ...
    plt.subplot(2, 3, 4)
    test_grouped = i_metrics.groupby('true_factors')[['sil', 'mean_r2', 's/n']].mean()
    ...
    plt.plot(test_grouped.index,
             test_grouped.index.map(lambda x: i_metrics[i_metrics['true_factors'] == x]['qtrue'].mean()),
             'o-', label='Qtrue')
    plt.plot(test_grouped.index,
             test_grouped.index.map(lambda x: i_metrics[i_metrics['true_factors'] == x]['qrobust'].mean()),
             's-', label='Qrobust')
    plt.axvline(x=test_k, color='r', linestyle='--')
    plt.xlabel('Factor Count')
    plt.ylabel('Q Value')
    ...
    accuracy_data = []
    training_df = pd.concat(optimizer.grouped_data.values(), ignore_index=True)
    for scenario_id in training_df['scenario_id'].unique()[:20]:
        scenario_data = training_df[training_df['scenario_id'] == scenario_id]
        true_factor = scenario_data['true_factors'].iloc[0]

        scenario_grouped = scenario_data.groupby('true_factors')['anomaly_score'].mean()
...
class MetricsAutoencoder(keras.Model):
...
def train_autoencoder(metrics_df, epochs=250, lr=1e-3, latent_dim=3):
    metrics = metrics_df.values.astype(np.float32)
    mean = metrics.mean(axis=0)
    std = metrics.std(axis=0) + 1e-8
    metrics_norm = (metrics - mean) / std

    input_dim = metrics.shape[1]
    model = MetricsAutoencoder(input_dim, latent_dim)
    model.compile(optimizer=keras.optimizers.Adam(lr), loss='mse')

    model.fit(metrics_norm, metrics_norm, epochs=epochs, verbose=0)

    recon = model.predict(metrics_norm)
    errors = np.mean((recon - metrics_norm) ** 2, axis=1)

    return errors, model, mean, std
...
if __name__ == "__main__":
    ...
    # true_k = np.random.random_integers(low=4, high=9)
    # optimizer = FactorCountOptimizer()
    # V, U, true_factors, noise_level = optimizer.generate_scenario(scenario_id=123, true_k=true_k)

    # metrics_df, used_true_factors = optimizer.run_scenario(V=V, U=U, true_factors=true_factors, k_min=3, k_max=10)
    # metrics_df.to_pickle('metrics_df_test.pkl')
    metrics_df = pd.read_pickle('metrics_df_test.pkl')

    recommended_cols = [
        'bcss', 'sil', 'mean_wcss', 'mean_r2', 's/n', 'qtrue', 'qrobust', 'membership_threshold'
    ]
    metrics_df_auto = metrics_df[recommended_cols]

    errors, model, mean, std = train_autoencoder(metrics_df_auto, epochs=500, lr=1e-3, latent_dim=2)
    optimal_idx = np.argmin(errors)
    optimal_factor_count = metrics_df.index[optimal_idx]
    true_factor = metrics_df.loc[optimal_factor_count, 'true_factors']

    print(f"Min Factors: 3-10")
    print(f"Errors: {errors}")
    print(f"Optimal Factor Count: {optimal_factor_count} - True Factor Count: {true_factor}")