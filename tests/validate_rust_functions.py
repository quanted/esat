    ...
    for method in ('ls-nmf', 'ws-nmf'):
        ...
        for i in _index:
            for j in range(0, V.shape[1]):
                ...
                U[i,j] = _U[i, j]
        ta0 = time.time()
        sa_py = SA(factors=n_components, V=V, U=U, seed=seed, method=method, optimized=False)
        sa_py.initialize()
        sa_py.train(max_iter=10000, converge_delta=0.01, converge_n=10)
        ta1 = time.time()
        _index = list(range(0, V.shape[0]))
        _V = copy.deepcopy(V[_index])
        _U = copy.deepcopy(U[_index])
        for i in _index:
            for j in range(0, V.shape[1]):
                ...
                U[i,j] = _U[i, j]
        tb0 = time.time()
        sa_r = SA(factors=n_components, V=V, U=U, seed=seed, method=method, optimized=True, parallelized=True)
        sa_r.initialize()
        sa_r.train(max_iter=10000, converge_delta=0.01, converge_n=10)
        tb1 = time.time()
        print(f"{method} - Python Q: {sa_py.Qtrue}, Rust Q: {sa_r.Qtrue}")
        print(f"{method} - Python runtime: {round((ta1-ta0), 2)} secs, Rust runtime: {round((tb1-tb0), 2)} secs")
